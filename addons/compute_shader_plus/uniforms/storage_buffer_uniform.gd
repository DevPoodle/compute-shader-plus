...
static func create(data: PackedByteArray) -> StorageBufferUniform:
	...
	uniform.storage_buffer = ComputeHelper.rd.storage_buffer_create(data.size(), data)
	return uniform
...
## Gets the buffer's data asynchronously.
## Returns a [Signal] with a [PackedByteArray] that will be emitted when the data is retrieved.
